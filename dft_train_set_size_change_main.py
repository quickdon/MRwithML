...
def main(npz_file, target='Etot'):
    ...
    for test_size in np.arange(0.1, 1, 0.1):
        if target not in ['Cp', 'Eatom']:
            train_data, X_test, y_test = stratified_split(df, target_column=target, test_size=test_size)
        else:
        ...
        print(f"Best Model Results: MSE={mse}, MAE={mae}, R2={r2}, test_size={test_size}")

        # 保存预测值与真实值对比图
        plot_comparison(y_test, y_pred, mse, mae, r2, target=target, title=f'Prediction vs Actual {target} (AutoGluon with {finger_name})', save_path=f'path/{test_size}_{finger_name}_{target}_autogluon_comparison.png')
